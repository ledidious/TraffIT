...
public class ConstructionSiteController extends AbstractController<ConstructionSite> {
	...
	DistanceTo<ConstructionSite> findInFront(@NonNull Vehicle vehicle) {
		return find(vehicle.getFrontCell(), Cell::getBlockingConstructionSite, false, MAX_CELLS_TO_SEARCH__INFINITE);
	}

	DistanceTo<ConstructionSite> findInFrontOnRightLane(@NonNull Vehicle vehicle) {

		if (vehicle.getLane().isTopRightLane()) {
			return null;
		}

		return find(vehicle.getFrontCell().getRightNeighbour(), Cell::getBlockingConstructionSite, false, MAX_CELLS_TO_SEARCH__INFINITE);
	}

	DistanceTo<ConstructionSite> findInBack(@NonNull Vehicle vehicle) {
		return find(vehicle.getTailCell(), Cell::getBlockingConstructionSite, true, MAX_CELLS_TO_SEARCH__INFINITE);
	}

	DistanceTo<ConstructionSite> findInBackOnRightLane(@NonNull Vehicle vehicle) {

		if (vehicle.getLane().isTopRightLane()) {
			return null;
		}

		return find(vehicle.getTailCell().getRightNeighbour(), Cell::getBlockingConstructionSite, true, MAX_CELLS_TO_SEARCH__INFINITE);
	}

	DistanceTo<ConstructionSite> findOnSameHeightOnRightLane(@NonNull Vehicle vehicle) {

		if (vehicle.getLane().isTopRightLane()) {
			return null;
		}

		return find(vehicle.getTailCell().getRightNeighbour(), Cell::getBlockingConstructionSite, false, vehicle.getLength());
	}

	DistanceTo<ConstructionSite> findOnSameHeightOnLeftLane(@NonNull Vehicle vehicle) {

		if (vehicle.getLane().isTopLeftLane()) {
			return null;
		}

		return find(vehicle.getTailCell().getLeftNeighbour(), Cell::getBlockingConstructionSite, false, vehicle.getLength());
	}
}
